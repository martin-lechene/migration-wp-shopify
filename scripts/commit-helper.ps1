...
# Référence le mini-plan de commits
...
function New-AtomicCommit {
    ...
    if (-not $commitMap.ContainsKey($Number)) {
        Write-Host "❌ Numéro de commit invalide: $Number" -ForegroundColor Red
    ...
    Write-Host ""
    Write-Host "🎯 Préparation du commit #$Number" -ForegroundColor Cyan
    Write-Host "=" * 80 -ForegroundColor Gray
    ...
    if ($currentBranch -eq $commit.Branch) {
        ...
        Write-Host ""
        Write-Host "⚠️  Vous n'êtes pas sur la bonne branche!" -ForegroundColor Yellow
    ...
    Write-Host "   1. Vérifiez que les fichiers correspondent à ce commit" -ForegroundColor White
    Write-Host "   2. Utilisez: git add <fichiers>" -ForegroundColor White
    Write-Host "   3. Puis exécutez à nouveau ce script pour valider le commit" -ForegroundColor White
    Write-Host ""
    
    $response = Read-Host "Voulez-vous créer le commit maintenant? (o/n)"
    if ($response -eq "o") {
        git add .
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host ""
            Write-Host "✅ Commit #$Number créé avec succès!" -ForegroundColor Green
            Write-Host ""
        } else {
            Write-Host ""
...
if ($List) {
    ...
    Write-Host "# Créer le commit #1" -ForegroundColor DarkGray
    Write-Host ""
